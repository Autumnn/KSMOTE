...
class metric_list():
    ...
    def measure(self, j_gamma, k_c, i_folder, Label_test, Label_predict):
        ...
        self.Precision[j, k, i] = metrics.precision_score(Label_test, Label_predict,average='macro')
        self.Recall[j, k, i] = metrics.recall_score(Label_test, Label_predict,average='macro')
        self.Specificity[j, k, i] = specificity_score(Label_test, Label_predict,average='macro')
        self.G_Mean[j, k, i] = geometric_mean_score(Label_test, Label_predict)
        self.F_Mean[j, k, i] = metrics.f1_score(Label_test, Label_predict,average='macro')

    def auc_measure(self, j_gamma, k_c, i_folder, Label_test, Label_score):
        j = j_gamma
        k = k_c
        i = i_folder
#        y = preprocessing.label_binarize(Label_test, classes=[-1,0,1])
#        print(y)
        self.AUC[j, k, i] = metrics.roc_auc_score(Label_test, Label_score,average='macro')
